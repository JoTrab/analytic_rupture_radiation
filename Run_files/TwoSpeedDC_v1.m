...
Trans.size_factor = 10; %reduce size to save memory and increase distance in z 
...
% % % % % % % Rupture.max_rup_steps = 250; %maximum time_points wished for calculation
% % % % % % %add a constant speed vector at start/end
% % % % % % Rupture.final_speed = 0.0001 ;%+0.0059%+50.005 % 0.027+(10.9*0.001) ; %add number of sources / points/ timesteps in meter that work at final speed
% % % % % % Rupture.start_speed = 0.00012%0.01
% % % % % % Rupture.length_total = Rupture.final_speed+Rupture.start_speed+0.005+0.01 
% % % % % % 
% % % % % % Rupture.length_total = 0.0001

% Rupture.length_total = Rupture.final_speed+Rupture.start_speed+0.005+0.006 %0.065 ;% in m accelerating and steady length
...
Source.Trise = 0.1 ;  %defines tris in ramp. make it 0.5 for triangle (gaussian), gaussianDC and Gaussian 0.1

% the smaller, the longer Source.signal_length
Source.d_shift_min = 0.022*Source.Trise_old ; % start  or end depends if accel or decel
Source.d_shift_min = 0.04*Source.Trise_old ; % start  or end depends if accel or decel
Source.d_shift_min = 0.012*Source.Trise_old ; % start  or end depends if accel or decel
Source.d_shift_min = 0.006*Source.Trise_old ; % start  or end depends if accel or decel
Source.d_shift_min = 0.012*Source.Trise_old ;
...
%show snapshots
figure; nexttile; imagesc(x2,y2, normalised_diff(-1*Ut(:,XposVector,25),99),[-1 1])
axis image
