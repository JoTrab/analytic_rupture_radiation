...
Trans.size_factor = 5; %reduce size and increase distance in z
...
Rupture.min_rup_time = 0.00001  ;
% Rupture.max_rup_steps = 250; %maximum time_points wished for calculation
Rupture.length_total = 0.002  ;% in m accelerating and steady length
%add a constant speed vector at start/end
Rupture.final_speed = 0.0000001 ; %add number of sources / points/ timesteps in meter that work at final speed
...
Params.p_speed = 1540;
Params.density = 1000;
Params.t_end = 0.05    ;% 0.025 ; %final simulation time  in seconds  - careful that not too short!!
...
Params.filename = strcat('Force_', num2str(Rupture.rup_speed_start_factor), '_', ... 
    ...
    Params.observation_plane, '_', num2str(Rupture.final_speed), '_', num2str(Params.calc_type) )  ;
...
if isequal(Params.testrun,'on') == 1
    ...
    if isequal(Params.Field_only,'on') 
        ...
        [Field_pad, Fieldterms, vt, Source, Params, Rupture,F_all_t] = wrapper_function(...
        Trans, Source, Params,  Rupture)   ;
    end
end
