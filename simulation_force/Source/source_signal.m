    ...
    case{'GaussDC', 'gausshape' }
        ...
        if isequal(type_switch, 'GaussDC') == 1
            ...
            if floor(Source.Trise*length(x)) == 0
                GaussDC = [gaussmf(x,[x(2) x(ceil(Source.GaussShift*length(x)))])];
                
            else
                GaussDC = [gaussmf(x,[x(floor(Source.Trise*length(x))) x(ceil(Source.GaussShift*length(x)))])];
            end
        ...
        elseif isequal(type_switch, 'gausshape') == 1
            ...
            if floor(0.1*length(x)) == 0
                signal = [gaussmf(x,[x(2) x(ceil(Source.GaussShift*length(x)))])];
                
            else
                signal = Mo*[gaussmf(x,[x(floor(Source.Trise*length(x))) x(ceil(Source.GaussShift*length(x)))])];
            end
            Amp = 'f [N]'
...
%         end
        ylabel('$\frac{\partial X_0}{\partial t} \; [\frac{N}{s}]$')
